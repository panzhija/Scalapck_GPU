...
#ifdef __STDC__
void hipPB_COutV( PBTYP_T * TYPE, char * ROWCOL, char * ZEROIT, int M,
               int N, int * DESCA, int K, char * * YAPTR, int * DYA,
               int * YAFREE, int * YASUM, int * BUFSIZE)
#else
void hipPB_COutV( TYPE, ROWCOL, ZEROIT, M, N, DESCA, K, YAPTR, DYA,
               YAFREE, YASUM, BUFSIZE )
/*
*  .. Scalar Arguments ..
*/
   char           * ROWCOL, * ZEROIT;
   int            * YAFREE, K, M, N, * YASUM, *BUFSIZE;
   PBTYP_T        * TYPE;
/*
*  .. Array Arguments ..
*/
...
*/
         if( Anq > 0 )
         {
            *YAPTR  = hipPB_Cmalloc( K * Anq * TYPE->size );
            *BUFSIZE = K * Anq;
            *YAFREE = 1;
...
*/
         if( Amp > 0 )
         {	
            *YAPTR  = hipPB_Cmalloc( Amp * K * TYPE->size );
            *BUFSIZE = Amp * K;
            *YAFREE = 1;
            if( Mupcase( ZEROIT[0] ) == CINIT )
            {
