...
#ifdef __STDC__
void hipPB_CpgemmBC( PBTYP_T * TYPE, char * DIRECB, char * DIRECC,
                  char * TRANSA, char * TRANSB, int M, int N, int K,
                  char * ALPHA, char * A, int IA, int JA, int * DESCA,
                  char * B, int IB, int JB, int * DESCB, char * BETA,
                  char * C, int IC, int JC, int * DESCC )
#else
void hipPB_CpgemmBC( TYPE, DIRECB, DIRECC, TRANSA, TRANSB, M, N, K, ALPHA,
...
*/
               ...
               {
                  Bbuf   = hipPB_Cmalloc( BnpD * nbb * size );
...
*/
            hipPB_CInV( TYPE, NOCONJG, ROW,    Am, An, Ad0, nbb, Bbuf, 0, 0,
                     DBUFB, &Broc, &WB, WBd, &WBfr );
/*
*  Allocate space for temporary results in scope of sub( A ) -> WC
*/
            hipPB_COutV( TYPE, COLUMN, INIT, Am, An, Ad0, nbb, &WC, WCd, &WCfr,
                      &WCsum, &WC_size);
/*
...
*/
               ...
               if( CisR || ( mycol == Ccurcol ) )
                  Cbuf = hipPB_Cmalloc( Cmp * nbb * size );
            }
            else
            {
/*
...
*  Replicate this panel of rows or columns of sub( B ) over sub( A ) -> WB
*/
            hipPB_CInV( TYPE, NOCONJG, COLUMN, Am, An, Ad0, nbb, Bbuf, 0, 0,
                     DBUFB, &Broc, &WB, WBd, &WBfr );
/*
*  Allocate space for temporary results in scope of sub( A ) -> WC
*/
            hipPB_COutV( TYPE, ROW,    INIT, Am, An, Ad0, nbb, &WC, WCd, &WCfr,
                      &WCsum, &WC_size);
/*
*  Local matrix-matrix multiply iff I own some data
*/
