...
#ifdef __STDC__
void hipPB_CInV( PBTYP_T * TYPE, char * CONJUG, char * ROWCOL, int M,
              int N, int * DESCA, int K, char * X, int IX, int JX,
              int * DESCX, char * XROC, char * * XAPTR, int * DXA,
              int * XAFREE )
#else
void hipPB_CInV( TYPE, CONJUG, ROWCOL, M, N, DESCA, K, X, IX, JX, DESCX,
...
*/
               ...
               else
               {
                  ...
                  if( Anq > 0 )
                  {
                     *XAPTR  = hipPB_Cmalloc( K * Anq * TYPE->size );
                     *XAFREE = 1;
                     top = PB_Ctop( &ctxt, BCAST, COLUMN, TOP_GET );
...
*/
      ...
      if( Xmp > 0 &&  Anq > 0 )
      {
         *XAPTR  = hipPB_Cmalloc( Anq * Xmp * TYPE->size );
         *XAFREE = 1;
      }
      if( XisRow )
      {
...
*/
               ...
               else
               {
                  ...
                  if( Amp > 0 )
                  {
                     *XAPTR = hipPB_Cmalloc( Amp * K * TYPE->size );
                     *XAFREE = 1;
                     top = PB_Ctop( &ctxt, BCAST, ROW, TOP_GET );
...
*/
      ...
      if( Xnq > 0 &&  Amp > 0 )
      {
         *XAPTR  = hipPB_Cmalloc( Amp * Xnq * TYPE->size );
         *XAFREE = 1;
      }
      if( XisRow )
      {
